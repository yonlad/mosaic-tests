...
class MosaicConfig:
    # Source
    image: str = "full-bod-imgs/prototypes/proto1.png"
    ...
    s3_bucket: str = os.getenv("REACT_APP_S3_BUCKET", "'pistoletto.test'")
    ...
    create_grey_background: bool = False
...
def match_and_place(src_rgb: np.ndarray, tiles: List[Tuple[int, int, int, int]], thumbs: List[Thumb], cfg: MosaicConfig) -> Image.Image:
    ...
    # flag to create a grey or white background
    if cfg.create_grey_background:
        background_color = (194, 189, 186)
    else:
...
def _parse_args() -> MosaicConfig:
    ...
    p.add_argument("--create-grey-background", action="store_true", default=MosaicConfig.create_grey_background, help="Create a grey background instead of white")
    p.add_argument(
